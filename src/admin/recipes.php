...
<head>
    ...
    <title>Rezepte verwalten - GreenGarnishLabs</title>
    <!-- Bootstrap CSS -->
        ...
        <div class="row">
            ...
            <div class="main-content">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <h2 class="mb-0">Rezepte verwalten</h2>
                    <a href="recipe-edit.php" class="btn btn-primary">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <form method="GET" class="row g-3">
                            <div class="col-md-4">
                                <input type="text" class="form-control" name="search" placeholder="Rezept suchen..." value="<?php echo htmlspecialchars($search); ?>">
                            </div>
                            <div class="col-md-3">
                ...
                <div class="recipe-list">
                    <?php foreach ($recipes as $recipe): ?>
                        <div class="recipe-item" data-recipe-id="<?php echo $recipe['id']; ?>">
                            <div class="row align-items-center">
                                <div class="col-md-4">
                                    <h5 class="mb-1"><?php echo htmlspecialchars($recipe['title']); ?></h5>
                                    <small class="text-muted">
                                        ID: <?php echo $recipe['id']; ?> | 
                                        Erstellt am: <?php echo date('d.m.Y H:i', strtotime($recipe['created_at'])); ?>
                                    </small>
                                </div>
                                <div class="col-md-2">
                                    <span class="status-badge <?php echo $recipe['status'] == 1 ? 'status-published' : 'status-draft'; ?>">
                                        <?php echo $recipe['status'] == 1 ? 'Veröffentlicht' : 'Entwurf'; ?>
                                    </span>
                                </div>
                                <div class="col-md-3">
                                    <small class="text-muted">
                                        <i class="bi bi-eye me-1"></i> <?php echo $recipe['views'] ?? 0; ?> Aufrufe
                                    </small>
                                </div>
                                <div class="col-md-3 text-end action-buttons">
                                    <a href="../recipes/view.php?id=<?php echo $recipe['id']; ?>" target="_blank" title="Vorschau">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="recipe-edit.php?id=<?php echo $recipe['id']; ?>" title="Bearbeiten">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <a href="#" onclick="deleteRecipe(<?php echo $recipe['id']; ?>)" title="Löschen" class="text-danger">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('confirmDelete').addEventListener('click', function() {
                if (itemToDelete) {
                    ...
                    .then(data => {
                        if (data.success) {
                            const element = document.querySelector(`[data-recipe-id="${id}"]`);
                            if (element) {
                                element.remove();
                            }
